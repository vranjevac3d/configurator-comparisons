...
function updateContactShadow(model) {
...
const externalMaps = {
  fabric: {
    normalMap: await loadTexture(`/${SKU}/${SKU}_FABRIC_Normal.jpg`),
    aoMap:     await loadTexture(`/${SKU}/${SKU}_FABRIC_AO.jpg`),
  },
  leather: {
...
gltfLoader.load(`/${SKU}/${SKU}.gltf`, (gltf) => {
  ...
  const materialType = "fabric"; // default; swap to "leather" to compare
  ...
  model.traverse((node) => {
    ...
    node.userData.externalMaps = externalMaps;

    if (materialWithBake(mat.name)) {
      const maps = externalMaps[materialType];
